...
# Check prerequisites - add common paths
export PATH="$PATH:/opt/conda/bin:/usr/local/bin"

if ! command -v node &> /dev/null; then
    echo "ERROR: Node.js not found. Please install Node.js 18+ or add it to PATH."
    exit 1
fi

if ! command -v npm &> /dev/null; then
    echo "ERROR: npm not found in PATH. Please ensure npm is installed and in PATH."
    exit 1
fi

if ! command -v zip &> /dev/null; then
    echo "ERROR: zip not found in PATH. Please install zip or add it to PATH."
    exit 1
fi

if ! command -v python3 &> /dev/null && ! command -v python &> /dev/null; then
    echo "ERROR: Python 3 not found in PATH. Please ensure Python 3.12+ is installed and in PATH."
    exit 1
fi

if ! command -v pip &> /dev/null && ! command -v pip3 &> /dev/null; then
    echo "ERROR: pip not found in PATH. Please ensure pip is installed and in PATH."
    exit 1
fi
